...
<body>
                                                            ...
                                                            <div class="alert alert-{{ $inquiry->result === 'true' ? 'success' : 'warning' }} mb-0">
                                                                <div class="d-flex justify-content-between align-items-start">
                                                                    ...
                                                                    <div class="d-flex gap-2 ms-3">
                                                                        <button type="button" class="btn btn-outline-primary btn-sm" onclick="viewFullResponse({{ $inquiry->id }})" title="View full official response">
    ...
    <script>
        ...
        console.log('=== PUBLIC INQUIRIES DEBUG ===');

        // Convert Laravel collection to JavaScript array properly
        let publicInquiries;
        try {
            const rawData = @json($publicInquiries);
            console.log('Raw PHP data:', rawData);
            console.log('Data type:', typeof rawData);

            // Handle both collection and array formats
            if (Array.isArray(rawData)) {
                publicInquiries = rawData;
            } else if (rawData && typeof rawData === 'object') {
                // Convert Laravel collection to array - handle different formats
                if (rawData.values && typeof rawData.values === 'function') {
                    publicInquiries = rawData.values();
                } else {
                    publicInquiries = Object.values(rawData);
                }
            } else {
                publicInquiries = [];
            }

            console.log('Processed publicInquiries:', publicInquiries);
            console.log('Count:', publicInquiries.length);
            if (publicInquiries.length > 0) {
                console.log('First inquiry sample:', publicInquiries[0]);
            }
        } catch (error) {
        ...
        function viewFullResponse(inquiryId) {
            console.log('=== VIEW FULL RESPONSE DEBUG ===');
            console.log('viewFullResponse called with ID:', inquiryId);
            console.log('ID type:', typeof inquiryId);
            console.log('Available inquiries:', publicInquiries);

            // Check if publicInquiries exists and is array
            if (!publicInquiries || !Array.isArray(publicInquiries)) {
                console.error('publicInquiries not available or not an array');
                alert('Error: Inquiry data not available. Please refresh the page.');
                return;
            }

            // Find the inquiry - handle both string and number IDs
            let inquiry = null;
            for (let i = 0; i < publicInquiries.length; i++) {
                const item = publicInquiries[i];
                if (item.id == inquiryId || item.id === inquiryId ||
                    String(item.id) === String(inquiryId)) {
                    inquiry = item;
                    break;
                }
            }
            ...
            if (!inquiry) {
                ...
                console.log('Available IDs:', publicInquiries.map(i => i.id));
            ...
            if (!modalBody) {
                ...
                alert('Error: Modal not properly configured.');
                return;
            }

            // Create modal content with safe handling of missing data
            const inquiryTitle = inquiry.title || 'No title available';
            const inquiryResult = inquiry.result || 'unknown';
            const adminResponse = inquiry.admin_response || 'No response available yet';
            const inquiryIdFormatted = String(inquiry.id).padStart(4, '0');
            ...
            modalBody.innerHTML = `
                    ...
                    <div class="col-12">
                        ...
                        <table class="table table-borderless mb-4">
                            <tr>
                                ...
                                <td><strong>#${inquiryIdFormatted}</strong></td>
                            </tr>
                            <tr>
                                <td class="text-muted">Title:</td>
                                <td><strong>${inquiryTitle}</strong></td>
                            </tr>
                            <tr>
                                <td class="text-muted">Verification Result:</td>
                                <td><span class="badge bg-${inquiryResult === 'true' ? 'success' : inquiryResult === 'false' ? 'danger' : 'secondary'}">${inquiryResult === 'true' ? 'TRUE' : inquiryResult === 'false' ? 'FAKE' : 'PENDING'}</span></td>
                            </tr>
                            <tr>
                                <td class="text-muted">Submission Date:</td>
                                <td>${inquiry.submission_date || 'Not available'}</td>
                            </tr>
                        </table>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12">
                        ...
                        <div class="alert alert-${inquiryResult === 'true' ? 'success' : inquiryResult === 'false' ? 'warning' : 'info'} mb-0">
                            <i class="material-icons" style="vertical-align: middle; margin-right: 8px;">verified</i>
                            <strong>Response from Verification Team:</strong>
                            <p class="mt-2 mb-0">${adminResponse}</p>
                        </div>
                    </div>
                </div>
            `;
            ...
            try {
                ...
                modal.show();
                console.log('Modal shown successfully for inquiry ID:', inquiryId);
            } catch (error) {
